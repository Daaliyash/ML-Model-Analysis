    ...
    if len(y.unique()) >= 10:
        ...
        with col7.container(border=True):
            ...
            for i in range(num_steps):
                ...
                colors.append((red, green, blue))
                if i == 0:
                    arr.append(df[dep_vars].min()[0])
                else:
                    arr.append(arr[i-1]+steps)
            for i in range(1, len(arr)):
                # arr[i]
                if rmse.round(3) <= arr[i]:
                    colors[i] = (0,0,0)
                    break
            plt.figure(figsize=(10, 2))
            ...
            st.slider('', float(df[dep_vars].min()[0]), float(df[dep_vars].max()[0]), float(rmse.round(3)), disabled=True)
                ...
                try:
                    ...
                    with col1:
                        ...
                        for num,column in enumerate(st.session_state.indep_vars):
                            number_input_value.append(st.number_input(column, st.session_state['filter_df'][column].min(), st.session_state['filter_df'][column].max(), st.session_state['filter_df'][column].mean()))
